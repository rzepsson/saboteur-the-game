...
export default function HowToPlayPanel() {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex bg-[#3e2406] px-5 sm:px-8 py-4 sm:py-5 border-b-4 border-[#5a360a] items-center gap-4 shrink-0">
        <div className="w-4 h-4 bg-[#a67c52] shrink-0" />
        <p className="text-[#e0c4a4] uppercase tracking-[0.15em] text-xl sm:text-2xl font-bold">
          {m.how_to_play_title()}
        </p>
      </div>

      {/* Tip content */}
      <div className="flex-1 min-h-0 p-4 sm:p-6 lg:p-8 flex flex-col gap-4 sm:gap-6">
        <AnimatePresence mode="wait">
          ...
          >
            <p className="text-[#a67c52] uppercase tracking-[0.15em] font-bold text-lg sm:text-xl">
              {m.tip_step_prefix()} {tip.step}
            </p>
            <h3 className="text-[#1e0e04] font-bold text-2xl sm:text-3xl leading-tight">
              {tip.titleFn()}
            </h3>
            <p className="text-[#5a360a] font-bold text-xl sm:text-2xl leading-relaxed">
              {tip.descFn()}
            </p>
          </motion.div>
        </AnimatePresence>
      ...
      {/* Navigation */}
      <div className="p-4 sm:p-6 lg:p-8 pt-0 flex items-center gap-3 sm:gap-5 shrink-0">
        <motion.button
          ...
          className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center bg-[#f8f0e0] border-4 border-[#5a360a] rounded-lg text-[#5a360a] disabled:opacity-40 hover:text-[#22c55e] hover:border-[#22c55e] transition-colors cursor-pointer disabled:pointer-events-none shrink-0"
        >
          <PixelChevronLeft className="w-6 h-6 sm:w-8 sm:h-8" />
        </motion.button>
        ...
        <div className="flex-1 flex items-center justify-center gap-3">
          {TIPS.map((_, i) => (
            <motion.button
              ...
              className={`h-5 sm:h-6 border-4 border-[#5a360a] rounded-sm cursor-pointer ${
        ...
        <motion.button
          ...
          className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center bg-[#f8f0e0] border-4 border-[#5a360a] rounded-lg text-[#5a360a] disabled:opacity-40 hover:text-[#22c55e] hover:border-[#22c55e] transition-colors cursor-pointer disabled:pointer-events-none shrink-0"
        >
          <PixelChevronRight className="w-6 h-6 sm:w-8 sm:h-8" />
        </motion.button>
      </div>
    </motion.div>
  );
